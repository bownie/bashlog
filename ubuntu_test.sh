...
echo "$(basename ${0}) INFO: ${random_string}$"
#grep -q -E "$(basename ${0})\[${$}\]: INFO: ${random_string}$" <<<"${syslogout}" \
  #&& result ok 'info -> syslog (/var/log/system.log)' \
  #|| result fail 'info -> syslog (/var/log/system.log)';
...
#grep -q -E $'^./log.sh: line [0-9]+: '"$(basename ${0})"'.log: Permission denied' <<<"${stderr}" \
  #&& grep -q -E $'\033\[31m[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[ERROR\] Logging Exception: echo -e "[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[INFO\] '"${random_string}"'" >> "'"$(basename ${0})"$'.log"\033\[0m$' <<<"${stderr}" \
...
#grep -q -E $'^./log.sh: line [0-9]+: '"$(basename ${0})"'.log.json: Permission denied' <<<"${stderr}" \
#  && grep -q -E $'\033\[31m[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[ERROR\] Logging Exception: echo -e "{"timestamp":"[0-9]{10}","level":"info","message":"'"${random_string}"'"}" >> "'"$(basename ${0})"$'.log.json"\033\[0m$' <<<"${stderr}" \
#  && result ok 'info -> file, Permission denied -> stderr' \
#  || result fail 'info -> file, Permission denied -> stderr';
...
#grep -q -E $'^./log.sh: line [0-9]+: '"$(basename ${0})"'.log: Permission denied' <<<"${stderr}" \
#  && grep -q -E $'\033\[31m[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[ERROR\] Logging Exception: echo -e "[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[WARN\] '"${random_string}"'" >> "'"$(basename ${0})"$'.log"\033\[0m$' <<<"${stderr}" \
...
#
##grep -q -E $'^./log.sh: line [0-9]+: '"$(basename ${0})"'.log: Permission denied' <<<"${stderr}" \
  #&& grep -q -E $'\033\[31m[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[ERROR\] Logging Exception: echo -e "[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} \[ERROR\] '"${random_string}"'" >> "'"$(basename ${0})"$'.log"\033\[0m$' <<<"${stderr}" \
